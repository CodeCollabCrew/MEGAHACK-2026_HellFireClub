...
const NAV = [
  ...
  { id: "excel"     as Tab, icon: FileSpreadsheet, label: "Excel"     },
];
...
export default function Sidebar({ activeTab, onTabChange, unreadCount, unprocessedCount, onProcessAll, processingEmails, onEmailsImported, user, onLogout }: SidebarProps) {
  ...
  const userSubtitle = user?.isGuest
    ? "Guest"
    : user?.email
      ? user.email.split("@")[0]
      : "";

...
router.get("/callback", async (req: Request, res: Response) => {
  ...
  try {
    ...
    console.log("Gmail token response:", tokens.error ? tokens : "OK");

    if (!tokens.access_token) {
      return res.redirect(`${frontendUrl}/dashboard?gmail_error=token_failed&detail=${tokens.error || "unknown"}`);
    }

    const result = await fetchGmailEmails(tokens.access_token, 30);
    console.log(`Gmail fetch result: imported ${result.imported}, errors: ${result.errors.length}`);
    res.redirect(`${frontendUrl}/dashboard?gmail_success=true&imported=${result.imported}`);
  } catch (err) {
...
// Debug endpoint — visit http://localhost:5000/api/gmail/debug in browser
router.get("/debug", async (_req: Request, res: Response) => {
  const count = await Email.countDocuments();
  const latest = await Email.find().sort({ receivedAt: -1 }).limit(3).select("subject from receivedAt isProcessed");
  sendSuccess(res, {
